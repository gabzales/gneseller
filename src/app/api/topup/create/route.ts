...
import { getActivePaymentGateway, createStenlyCharge } from "@/lib/provider/stenly";
...
export async function POST(request: Request) {
  ...
  if (activeGateway === "genspay" && (!GENSPAY_BASE_URL || !GENSPAY_API_KEY)) {
    return NextResponse.json(
      {
  ...
  let qrString: string;
  let amount: number;
  let expiredAt: string | number | null;

  if (activeGateway === "stenly") {
    // stenly.id doesn't take callback_url per-request -- its callback URL
    // is configured per-project in their own dashboard (see docs), so
    // there's nothing to send here; make sure the project's Callback URL
    // there is set to ${appUrl}/api/webhooks/stenly.
    const chargeResult = await createStenlyCharge({
      orderId,
      grossAmount: pkg.nominal,
      customerName: user.user_metadata?.name as string | undefined,
      customerEmail: user.email ?? undefined,
    });
    if (!chargeResult.success) {
      return NextResponse.json({ error: "gateway_rejected", message: chargeResult.message }, { status: 502 });
    }
    qrString = chargeResult.qrString;
    amount = pkg.nominal;
    expiredAt = chargeResult.expiresAt;
  } else {
  ...
  if (pendingError) {
    ...
    // The gateway transaction still exists and the webhook will retry
    // settlement regardless -- don't block the checkout redirect over
